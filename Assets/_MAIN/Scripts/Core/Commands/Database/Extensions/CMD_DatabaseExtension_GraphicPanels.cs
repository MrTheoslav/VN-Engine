...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_GraphicPanels : CMD_DatabaseExtension
    {
        ...
        public static IEnumerator SetLayerMedia(string[] data)
        {
            ...
            //Try to get speed of the transition if it is not an immediate effect
            if (!immediate)
            ...
            //Now run the logic
            pathToGraphic = FilePaths.GetPathResource(FilePaths.resources_backgroundImages, mediaName);
            graphic = Resources.Load<Texture>(pathToGraphic);

            if (graphic == null)
            {
                pathToGraphic = FilePaths.GetPathResource(FilePaths.resources_backgroundVideos, mediaName);
                graphic = Resources.Load<VideoClip>(pathToGraphic);
            }

            if (graphic == null)
            {
                Debug.LogError($"Could not find media file called '{mediaName}' in the Resources directories. Please specify the full path resources and make sure that the file exists!");
                yield break;
            }
        ...
        public static IEnumerator ClearLayerMedia(string[] data)
        {
            //Parameters available to function
            string panelName = "";
            int layer = 0;
