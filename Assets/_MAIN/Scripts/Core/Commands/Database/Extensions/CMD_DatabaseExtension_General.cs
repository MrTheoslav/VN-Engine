...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_General : CMD_DatabaseExtension
    {
        ...
        private static void LoadNewDialogueFile(string[] data)
        {
            ...
            string filePath = FilePaths.GetPathResource(FilePaths.resources_dialogueFiles, fileName);
            TextAsset file = Resources.Load<TextAsset>(filePath);

            if (file == null)
            {
                Debug.LogWarning($"File '{filePath}' could not be loaded from dialogue files. Please ensure it exists ithin the '{FilePaths.resources_dialogueFiles}' resources folder.");
                return;
            }
        ...
        private static IEnumerator ShowDialogueBox(string[] data)
        {
            ...
            //Try to get speed Showing Dialogue System
            parameters.TryGetValue(data, out speed, defaultValue: 1);

            //Try to get if show immediately
            parameters.TryGetValue(data, out immediate, defaultValue: false);
