...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Audio : CMD_DatabaseExtension
    {
        ...
        private static void PlaySFX(string[] data)
        {
            ...
            //Try to get if this sound loop
            parameters.TryGetValue(PARAM_LOOP, out loop, defaultValue: false);

            //Run the logic
            string resourcesPath = FilePaths.GetPathResource(FilePaths.resources_sfx, filepath);
            AudioClip sound = Resources.Load<AudioClip>(resourcesPath);

            if (sound == null)
            {
                Debug.LogError($"Was not able to load sfx '{filepath}'");
                return;
            }
        ...
        private static void PlayVoice(string[] data)
        {
            ...
            //Run the logic
            AudioClip sound = Resources.Load<AudioClip>(FilePaths.GetPathResource(FilePaths.resources_voices, filepath));
        ...
        private static void PlaySong(string[] data)
        {
            ...
            filePath = FilePaths.GetPathResource(FilePaths.resources_music, filePath);
        ...
        private static void PlayAmbience(string[] data)
        {
            ...
            filePath = FilePaths.GetPathResource(FilePaths.resources_ambience, filePath);
