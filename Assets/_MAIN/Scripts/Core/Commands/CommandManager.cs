    ...
    public class CommandManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (instance == null)
            {
                ...
                Type[] extentionTypes = assembly.GetTypes().Where(t => t.IsSubclassOf(typeof(CMD_DatabaseExtension))).ToArray();

                foreach (Type extention in extentionTypes)
                {
                    MethodInfo extendMethod = extention.GetMethod("Extend");
        ...
        private CoroutineWrapper ExecuteSubCommand(string commandName, string[] args)
        {
            ...
            //If we've made down here then we should try to run as a character command
