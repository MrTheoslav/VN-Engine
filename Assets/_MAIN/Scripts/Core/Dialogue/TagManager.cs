...
public class TagManager
{
    ...
    private static string InjectVariables(string value)
    {
        ...
        for (int i = matchesList.Count - 1; i >= 0; i--)
        {
            ...
            bool endsInIllegalCharacter = variableName.EndsWith(VariableStore.DEFAULT_DATABASE_RELATIONAL_ID);
            ...
            int lenghtToBeRemove = match.Index - match.Length > value.Length ? value.Length - match.Index : match.Length;
            if (endsInIllegalCharacter)
                lenghtToBeRemove -= 1;

            value = value.Remove(match.Index, lenghtToBeRemove);
