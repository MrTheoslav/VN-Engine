...
namespace VISUALNOVEL
{
    ...
    public class VNGameSave
    {
        ...
        public const string FILETYPE = ".vns";
        ...
        public string filePath => $"{FilePaths.gameSaves}{slotNumber}{FILETYPE}";
        ...
        public static string GetScreenshotFilePath(int fileNumber) => $"{FilePaths.gameSaves}{fileNumber}{SCREENSHOT_FILE_TYPE}";
        ...
        public void Save()
        {
            ...
            activeState = HistoryState.Caputre();
        ...
        private void SetConversationData()
        {
            for (int i = 0; i < activeConversations.Length; i++)
            {
                ...
                {
                    Debug.LogError($"Encoutered error while extracting saved conversation data! {e}");
                    continue;
                }
            }
        }
        ...
        private void SetVariableData()
        {
            foreach (var variable in variables)
            {
                ...
                {
                    case "System.Boolean":
                        if (bool.TryParse(val, out var b_val))
                        {
                        ...
                        break;
                    case "System.Int32":
                        if (int.TryParse(val, out var i_val))
                        {
                        ...
                        break;
                    case "Sytem.Single":
                        if (float.TryParse(val, out var f_val))
                        {
